...
function deleteFrames(state, payload, blockInfo, context) {
  Storage.deleteMany({ stamp_secs: { $gt: payload.data.minTime, $lte: payload.data.maxTime}, hash_uid: payload.data.hash_rfid }, function (req, response, err) {
    if (err) console.log(err);
    console.log(response);
  });
}

function clearStorage(state, payload, blockInfo, context) {
  Storage.deleteMany({}, function (req, response, err) {
    if (err) console.log(err);
    console.log(response);
  });
}
...
const updaters = [
  {
    actionType: "storage::eraseall",
    apply: clearStorage,
  },
  {
    actionType: environment.eosio_contract_account_storage + "::" + environment.eosio_contract_account_storage_action_erase,
    apply: deleteFrames,
  },
]
