...
export class TimeMaskDirective
  ...
  writeValue(value: Date): void {
    if (value && !(value instanceof Date)) {
      throw new Error(
        "A diretiva appTimeMask exige que o valor do componente seja do tipo Date"
      );
    }
  ...
  validate(c: FormControl): { [key: string]: any } {
    return this._el.nativeElement.value.indexOf("-") === -1
      ? null
      : { validTime: false };
  }
  ...
  private _controlValueChanged() {
    ...
    this._dateValue = new Date(
      this._dateValue.setHours(this._stringToNumber(timeArray[0]))
    );
    this._dateValue = new Date(
      this._dateValue.setMinutes(this._stringToNumber(timeArray[1]))
    );
